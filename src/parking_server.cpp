    ...
    while (true) {
        try {
            ...
            if (status == sf::Socket::Disconnected) {
                std::cout << "Client disconnected (thread: " << std::this_thread::get_id() << ")\n";
                break;
            }
            ...
            if (command == "ENTER") {
                ...
                if (lot.ExitVehicle(number, cost, totalRevenue)) {
                    response = "OK COST " + std::to_string(round(cost)) + " REVENUE " + std::to_string(totalRevenue);
                } else {
                    response = "ERROR Vehicle " + number + " not found";
                }
            }
            else if (command == "STATUS") {
                response = "STATUS free=" + std::to_string(lot.getFreeSpots()) + " revenue=" + std::to_string(lot.getRevenue());
            }
