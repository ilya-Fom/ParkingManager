...
ParkingLot::ParkingLot(const std::string& path, const int price) : hourlyRate(price), revenue(0.0f) {
...
bool ParkingLot::EnterVehicle(const std::string &vehicleNumber, float vehicleSize, int &SpotId) {
...
bool ParkingLot::ExitVehicle(const std::string &vehicleNumber, float &cost, float &totalRevenue) {
    std::lock_guard<std::mutex> lock(mtx);
    for (ParkingSpot &spot : Spots) {
