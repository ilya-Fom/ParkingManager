...
ParkingSpot::ParkingSpot(int inputid, float inputsize) : id(inputid), size(inputsize), occupied(false) {}
...
double ParkingSpot::leave() {
    ...
    throw std::logic_error("The spot " + std::to_string(id) + " is already empty");
}
...
float ParkingSpot::getSize() const {
    return size;
}
