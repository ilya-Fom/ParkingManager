...
private:
    ...
    std::string VehicleNumber; ///< Госномер припаркованного ТС (пусто, если свободно)
    std::time_t entryTime; ///< Время въезда в формате time_t (для расчёта стоимости)
public:
    /**
     ...
     * 3. Переводит секунды в часы (деление на 60 — каждая минута считается за час)
     ...
     * @return const std::string& Ссылка на строку с номером (пусто, если свободно)
     */
